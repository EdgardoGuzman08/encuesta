...
if (isset($_GET['uuid'])) {
    ...
    $sqlVerificar = "SELECT escaneado, intentos_fallidos FROM encuestas WHERE codigo_qr LIKE '%codigo_qr_$uuid%.png' AND activo = 1";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($escaneado == 1) {
            ...
            echo "<script>mostrarAlerta('Este código QR ya ha sido escaneado.');</script>";
            
            // Incrementa el contador de intentos fallidos
            $intentosFallidos++;

            // Actualiza el campo "intentos_fallidos" en la base de datos
            $sqlActualizarIntentos = "UPDATE encuestas SET intentos_fallidos = $intentosFallidos WHERE codigo_qr LIKE '%codigo_qr_$uuid%.png'";
            $conn->query($sqlActualizarIntentos);
        ...
        echo "<script>mostrarAlerta('Código QR inválido o no disponible.');</script>";
        exit;
    }
}
